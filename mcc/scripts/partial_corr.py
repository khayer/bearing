...
def main():
    ...
    ap.add_argument("--region", default=None,
                    help="chrom:start-end to restrict bins (maps where the sign lives)")
    args = ap.parse_args()
    path = args.dump
    ...
    keep = rows[1:]
    if args.region:
        rc, rng = args.region.split(":")
        ra, rb = (int(x) for x in rng.split("-"))
        keep = [r for r in keep if r[idx["chrom"]] == rc
                and int(r[idx["end"]]) > ra and int(r[idx["start"]]) < rb]
        if not keep:
            sys.exit("[ERROR] no bins in region")
    data = np.array([[float(r[idx[c]]) for c in ("bes", "delta_contact",
                     "marginal", "width", "on_feature")] for r in keep])
    ...
    print("file              : %s%s" % (path,
          ("  region=%s" % args.region) if args.region else ""))
