...
def main():
    ...
    c, bes, delta = load_region(args.dump, chrom, a, b)
    if len(c) == 0:
    ...
    axb.set_title("%s   |   Spearman(BES, dContact)=%.3f%s"
                  % (args.title, rho,
                     ("   partial|depth=%.2f" % args.partial_rho)
                     if args.partial_rho is not None else ""), fontsize=11)
    ...
    sys.stderr.write("[fig] %s  (%d bins, rho=%.3f)\n" % (args.out, len(c), rho))
